 ...
 * 
 * Adds methods for accessing referenced documents on other collections.
 * For example, with post.authorId, a virtual 'post.author' and 'post.getAuthor()' will be added, 
 * which will return the document from the Author collection.
 * 
 * When a referenced document is fetched, it is cached locally so that 'post.author' can be used without
 * further DB hits.
 *
 * Required parameters:
 * idKey:
 *      Description : The name for the key where the referenced document ID is e.g. 'authorId'
 *      Type        : String
 * 
 * virtualKey:
 *      Description : The name for the virtual that will be added. E.g. 'post.author'
 *      Type        : String
 *
 * modelName:
